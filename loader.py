...
class MoleculeDataset(InMemoryDataset):
    def __init__(self,
        ...
        fp_path = os.path.join(root, "rdkfp1-7_512.npz")
        md_path = os.path.join(root, "molecular_descriptors.npz")
    ...
    def process(self):
        ...
        if 'CHEMBL' in self.dataset:
            smiles_list, rdkit_mol_objs, labels = \
                _load_chembl_dataset(self.raw_paths[0])
            for i in range(len(smiles_list)):
                print(i)
                rdkit_mol = rdkit_mol_objs[i]
                # # convert aromatic bonds to double bonds
                ...
                # manually add mol id
                # data.id = torch.tensor(
                #     [i])  # id here is the index of the mol in the dataset
                # data.smiles = smiles_list[i]
                # data.y = torch.tensor([labels[i]], dtype=torch.float).unsqueeze(0)
                data_list.append(data)
                data_smiles_list.append(smiles_list[i])
            ...
            for i in range(len(smiles_list)):
                ...
                # the dataset
                data.smiles = smiles_list[i]
                data.y = torch.tensor([labels[i]], dtype=torch.long).unsqueeze(0)
                data_list.append(data)
                data_smiles_list.append(smiles_list[i])
...
def _load_chembl_dataset(input_path):
    input_df = pd.read_csv(input_path, sep=',')
    smiles_list = input_df['smiles']
    rdkit_mol_objs_list = [AllChem.MolFromSmiles(s) for s in smiles_list]
    labels = input_df['y']
...
def create_all_datasets(args):
    ...
    if args.task is None:
        ...
        downstream_dir = [args.task]

    for dataset_name in downstream_dir:
        print(dataset_name)
        root = os.path.join(args.root_dir, dataset_name)
...
def split_all_datasets(args):
    ...
    for dataset_name in downstream_dir:
        ...
        if args.split_mode == "random":
            train, dev, test = random_split(dataset, frac_train=0.8, frac_valid=0.1, frac_test=0.1, seed=args.seed)
        elif args.split_mode == "random_scaffold":
            train, dev, test = random_scaffold_split(dataset, dataset.smiles, frac_train=0.8, frac_valid=0.1, frac_test=0.1, seed=args.seed)
        elif args.split_mode == "scaffold":
            train, dev, test = scaffold_split(dataset, dataset.smiles, task_idx=None, null_value=0,
                                   frac_train=0.8,frac_valid=0.1, frac_test=0.1)
        elif args.split_mode == "random_stratified":
            train, dev, test = random_stratified_split(dataset, dataset.smiles, frac_train=0.8, frac_valid=0.1, frac_test=0.1, seed=args.seed)
        else:
            raise NotImplementedError
        path = os.path.join(root, "processed", "{}_{}".format(args.split_mode, args.seed), "train.pt")
        # fs.torch_save(train, path)
        _save(train, path)
        path = os.path.join(root, "processed", "{}_{}".format(args.split_mode, args.seed), "dev.pt")
        # fs.torch_save(dev, path)
        _save(dev, path)
        path = os.path.join(root, "processed", "{}_{}".format(args.split_mode, args.seed), "test.pt")
        # fs.torch_save(test, path)
        _save(test, path)
        print(dataset)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--root_dir", type=str, default="data/downstream_tasks/")
    parser.add_argument("--split_mode", type=str, default="random", choices=["random", "random_scaffold","scaffold","random_stratified"])
    ...
    create_all_datasets(args)
    split_all_datasets(args)