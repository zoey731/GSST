...
from model.blip2_qinit import Blip2Qinit
from data_provider.qinit_dm import QInitDM
...
def main(args):
    ...
    if args.init_checkpoint:
        model = Blip2Qinit.load_from_checkpoint(args.init_checkpoint, device=args.devices, args=args)
        print(f"loading model from {args.init_checkpoint}")
    else:
        model = Blip2Qinit(args)
    ...
    if args.root.find('kv') >= 0:
        dm = Stage1KVPLMDM(args.num_workers, args.batch_size, args.root, args.max_length, args.graph_aug, args)
    else:
        dm = QInitDM(args.num_workers, args.batch_size, args.root, args.max_length, args.graph_aug, tokenizer, args)
...
if __name__ == '__main__':
    ...
    parser = Blip2Qinit.add_model_specific_args(parser)  # add model args
    parser = QInitDM.add_model_specific_args(parser)
