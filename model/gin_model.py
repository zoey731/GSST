...
class GNN(torch.nn.Module):
    ...
    def __init__(self, num_layer, emb_dim, JK = "last", drop_ratio = 0, gnn_type = "gin"):
        ...
        self.act_fn = nn.SiLU()
        self.gate = nn.Linear(emb_dim+200+512, emb_dim)
        self.modified = nn.Linear(emb_dim+200+512, emb_dim)
    ...
    def forward(self, *argv):
        if len(argv) == 3:
            ...
            data = argv[0]
            x, edge_index, edge_attr, batch, fp, md = data.x, data.edge_index, data.edge_attr, data.batch, data.fp, data.md
        else:
            raise ValueError("unmatched number of arguments.")

        x = self.x_embedding1(x[:,0]) + self.x_embedding2(x[:,1])##原子类型和手性
        ...
        fp = fp.view(-1, 512)
        md = md.reshape(-1,  200) 
        assert fp.shape[0]  == md.shape[0]  == h_graph.shape[0]
        h_graph = torch.cat([h_graph,  fp, md], dim=1)  ##多尺度特征融合，实现全局-局部特征的物理拼接 
        h_graph = self.act_fn(self.gate(h_graph)) * self.modified(h_graph)
        # h_graph = self.modified(h_graph)
    ...
    def forward_old(self, *argv):##原本的forward函数
        if len(argv) == 3:
            x, edge_index, edge_attr = argv[0], argv[1], argv[2]
        elif len(argv) == 1:
            data = argv[0]
