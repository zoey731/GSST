import torch
from model.blip2qmole import Blip2QMole
...
class Blip2Qinit(pl.LightningModule):
    def __init__(self, args):
        ...
        self.blip2qformer = Blip2QMole(args.gtm, args.lm, args.bert_name, args.temperature, args.gin_num_layers, args.gin_hidden_dim, args.drop_ratio, args.tune_gnn, args.num_query_token, args.cross_attention_freq, args.projection_dim)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        ###============== Overall Loss ===================###
        self.log("val_loss_gtc", float(blip2_loss.loss_itc), batch_size=batch_size, prog_bar=True, sync_dist=True)
        self.log("val_loss_gtm", float(blip2_loss.loss_itm), batch_size=batch_size, prog_bar=True, sync_dist=True)
        self.log("val_loss", float(blip2_loss.loss), batch_size=batch_size, prog_bar=True, sync_dist=True)
    ...
    def on_validation_epoch_end(self) -> None:
        ...
        if self.trainer.global_rank == 0:
            ...
            graph_rep_total, smiles_rep_total, _, _, _, _ = \
            ...
            g2t_acc, g2t_rec20, t2g_acc, t2g_rec20, _ = \
                eval_retrieval_fullset(graph_rep_total, smiles_rep_total, self.device)
            ...
            # g2t_acc, t2g_acc, g2t_rec20, t2g_rec20, \
            # g2t_rerank_acc, t2g_rerank_acc, g2t_rerank_rec20, t2g_rerank_rec20, \
            # graph_rep_total, smiles_rep_total, graph_feat_total, graph_mask_total, smiles_total, smiles_mask_total = \
            ...
            #     eval_retrieval_fullset_for_rerank(self.blip2qformer, sim_g2t, graph_feat_total, graph_mask_total, smiles_total, smiles_mask_total, self.rerank_cand_num, self.device)
            ...
            del graph_rep_total, smiles_rep_total
    ...
    def training_step(self, batch, batch_idx):
        ...
        blip2_loss = self.blip2qformer(batch)
        ###============== Overall Loss ===================###
        self.log("train_loss_gtc", float(blip2_loss.loss_itc), batch_size=batch_size, prog_bar=True, sync_dist=True)
        self.log("train_loss_gtm", float(blip2_loss.loss_itm), batch_size=batch_size, prog_bar=True, sync_dist=True)
        self.log("train_loss", float(blip2_loss.loss), batch_size=batch_size, prog_bar=True, sync_dist=True)
...
def eval_retrieval_inbatch(model, dataloader, device=None):
    assert isinstance(model, Blip2QMole)
    model.eval()
    g2t_acc = 0
    t2g_acc = 0
    g2t_rec20 = 0
    ...
    for batch in tqdm(dataloader):
        aug, smiles, smiles_mask = batch
        smiles_total.append(smiles)
        smiles_mask_total.append(smiles_mask)

        aug = aug.to(device)
        smiles = smiles.to(device)
        smiles_mask = smiles_mask.to(device)
        graph_rep, graph_feat, graph_mask = model.graph_forward(aug) # shape = [B, num_qs, D]
        smiles_rep = model.smiles_forward(smiles, smiles_mask) # shape = [B, D]

        sim_q2t = (graph_rep.unsqueeze(1) @ smiles_rep.unsqueeze(-1)).squeeze() # shape = [B, 1, num_qs, D]; shape = [B, D, 1]; output shape = [B, B, num_qs]
        ...
        smiles_rep_total.append(smiles_rep.cpu())
    ...
    smiles_rep_total = torch.cat(smiles_rep_total, dim=0)
    graph_feat_total = pad_and_concat(graph_feat_total)
    graph_mask_total = pad_and_concat(graph_mask_total)
    smiles_total = torch.cat(smiles_total, dim=0)
    smiles_mask_total = torch.cat(smiles_mask_total, dim=0)
    ...
    return g2t_acc, t2g_acc, g2t_rec20, t2g_rec20, graph_rep_total, smiles_rep_total, graph_feat_total, graph_mask_total, smiles_total, smiles_mask_total


@torch.no_grad()
def eval_retrieval_fullset(graph_rep, smiles_rep, device):    
    N = graph_rep.shape[0]
    B = 8
    ...
    for i in tqdm(range(0, N, B)):
        ...
        l_sim_q2t = (l_graph_rep.unsqueeze(1) @ smiles_rep.unsqueeze(-1)).squeeze() # shape = [B, 1, num_qs, D]; shape = [N, D, 1]; output shape = [B, N, num_qs]
...
@torch.no_grad()
def eval_retrieval_fullset_for_rerank(model, sim_g2t_total, graph_feat_total, graph_mask_total, smiles_total, smiles_mask_total, rerank_cand_num, device):
    ...
    for i in tqdm(range(0, N, B), desc='re-ranking g2t'):
        ...
        smiles = smiles_total[topk_idx].flatten(0,1).to(device) # shape = [B * rcn, smiles_len]
        smiles_mask = smiles_mask_total[topk_idx].flatten(0,1).to(device) # shape = [B * rcn, smiles_len]
        gtm_sim = model.compute_gtm(graph_feat, graph_mask, smiles, smiles_mask).reshape(rB, rcn) ## fixme, using the linear clf's logits directly, without softmax
    ...
    for i in tqdm(range(0, N, B), desc='re-ranking t2g'):
        ...
        topk_idx = topk_idx.cpu()
        smiles = smiles_total[i:i+B].to(device).repeat_interleave(rcn, 0)
        smiles_mask = smiles_mask_total[i:i+B].to(device).repeat_interleave(rcn, 0)
        graph_feat = graph_feat_total[topk_idx].to(device).flatten(0,1)
        graph_mask = graph_mask_total[topk_idx].to(device).flatten(0,1)
        gtm_sim = model.compute_gtm(graph_feat, graph_mask, smiles, smiles_mask).reshape(rB, rcn)
        sorted_ids = torch.argsort(topk_sim + gtm_sim, descending=True).cpu() # shape = [B, rcn]
...
def eval_retrieval_inbatch_with_rerank(model, dataloader, device=None):
    ...
    for batch in tqdm(dataloader):
        ...
        smiles = smiles.repeat(B, 1) # shape = [B * B, smiles_len]
        smiles_mask = smiles_mask.repeat(B, 1) # shape = [B * B, smiles_len]

        if False:
            gtm_sim = model.compute_gtm(graph_feat, graph_mask, smiles, smiles_mask).reshape(B, B)
        else:
            ## batched reranking
            batch_size = 4
            gtm_sim = []
            for i in range(0, graph_feat.shape[0], batch_size):
                gtm_sim_local = model.compute_gtm(graph_feat[i:i+batch_size], graph_mask[i:i+batch_size], smiles[i:i+batch_size], smiles_mask[i:i+batch_size])
